...
function statusClass(status: string): string {
  ...
  if (["done", "won", "approved", "processed"].includes(s)) return "text-green-700 bg-green-100";
  if (["pending"].includes(s)) return "text-blue-700 bg-blue-100";
  if (["lost/settled", "deadline missed", "denied"].includes(s)) return "text-red-700 bg-red-100";
...
export default function EscalationsPanel() {
  ...
  // admin lands on the Chat Drivers report; agents go straight to Chargeback.
  const [view, setView] = useState<"drivers" | "chargeback" | "refund">(isAdmin ? "drivers" : "chargeback");

  if (!isAdmin && !isAgent) {
    return <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 text-sm text-gray-500">Not available for this role.</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        {isAdmin && <button onClick={() => setView("drivers")} className={tab(view === "drivers")}>Chat Drivers</button>}
        <button onClick={() => setView("chargeback")} className={tab(view === "chargeback")}>Chargeback</button>
        <button onClick={() => setView("refund")} className={tab(view === "refund")}>Refund</button>
      </div>
      {view === "drivers" ? (
        <ChatDriversView />
      ) : (
        <EscalationCrud key={view} formId={view} isAdmin={isAdmin} />
      )}
    </div>
  );
}

// ─── Chat Drivers (read-only, auto from per-chat tagging) — admin only ───
...
// ─── Chargeback / Refund (manual CRUD) ───────────────────────────────────
// Agents: create + see their own only. Admin: see all, with an Agent column.
function EscalationCrud({ formId, isAdmin }: { formId: string; isAdmin: boolean }) {
  const FORM = getForm(formId)!;
  const STATUS = FORM.fields.find((f) => f.key === "status")?.options ?? [];
  const propertyOptions = FORM.fields.find((f) => f.key === "property")?.options ?? [];
  ...
  }, [formId, fProperty, fStatus, fQuery]);

  useEffect(() => { load(); }, [load]);

  const agentOf = (r: EscalationRecord) => r.agentName || r.createdBy || "";
  const agentOptions = Array.from(new Set(records.map(agentOf).filter(Boolean))).sort();
  const visible = isAdmin && fAgent ? records.filter((r) => agentOf(r) === fAgent) : records;
  ...
  async function save() {
    ...
    try {
      const res = editingId
        ...
        : await fetch("/api/escalations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ formId, data: formData }) });
  ...
  async function remove(id: string) {
    if (!confirm(`Delete this ${FORM.title.toLowerCase()} record?`)) return;
    try {
  ...
  function downloadCsv() {
    ...
    const header = [...cols.map((c) => c.label), ...(isAdmin ? ["Agent"] : []), "Created"];
    const lines = [header.map(escapeCSV).join(",")];
    for (const r of visible) {
      const row = [...cols.map((c) => escapeCSV(r.data[c.key] ?? "")), ...(isAdmin ? [escapeCSV(agentOf(r))] : []), escapeCSV(fmtDate(r.createdAt))];
      lines.push(row.join(","));
    }
    downloadCSV(`${formId}_${today()}.csv`, lines.join("\n"));
  }

  const hasFilters = !!(fProperty || fStatus || fAgent || fQuery);
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          {isAdmin && (
            <div>
              ...
              <select value={fAgent} onChange={(e) => setFAgent(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 max-w-[180px]">
          ...
          {hasFilters && <button onClick={() => { setFProperty(""); setFStatus(""); setFAgent(""); setFQuery(""); }} className="text-sm text-gray-500 hover:text-gray-700 underline pb-2">Clear</button>}
          <button onClick={openCreate} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">+ New {FORM.title}</button>
        </div>
        {error && <p className="mt-3 text-sm text-red-600">{error}</p>}
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
        <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">
            {FORM.title} — {visible.length}
            {loading && <span className="ml-2 text-sm font-normal text-gray-400">loading…</span>}
            {!isAdmin && <span className="ml-2 text-xs font-normal text-gray-400">your records</span>}
          </h3>
          {visible.length > 0 && <button onClick={downloadCsv} className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-700">Download CSV</button>}
        </div>
        {visible.length === 0 ? (
          <div className="text-center py-12 text-gray-400">{loading ? "Loading…" : `No ${FORM.title.toLowerCase()} records. Click “New ${FORM.title}”.`}</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-900 text-white">
                  {FORM.fields.map((f) => <th key={f.key} className="px-3 py-2 text-left font-medium whitespace-nowrap">{f.label}</th>)}
                  {isAdmin && <th className="px-3 py-2 text-left font-medium whitespace-nowrap">Agent</th>}
              ...
              <tbody>
                {visible.map((r, i) => (
                  <tr key={r.id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    {isAdmin && <td className="px-3 py-2 whitespace-nowrap">{agentOf(r) || <span className="text-gray-300">—</span>}</td>}
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg my-8">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">{editingId ? "Edit" : "New"} {FORM.title}</h3>
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-200">
              ...
              <button onClick={save} disabled={saving} className="px-5 py-2 rounded-lg text-sm font-medium bg-gray-900 text-white hover:bg-gray-800 disabled:opacity-50">{saving ? "Saving…" : editingId ? "Save Changes" : `Create ${FORM.title}`}</button>
            </div>
          </div>
        </div>
      )}
